...
contract Whitelist is Ownable, ERC721Enumerable {
    ...
    TokenAndFee[] public tokensAccepted;
    ...
    constructor(
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            tokenAccepted[_tokens[i].token] = _tokens[i];
        }
    }
    ...
    function buyWhitelistSpot(address token) public {
        ...
        require(
            ...
            "You already have a whitelist spot."
        );
    ...
    function gimmeARefund(uint256 nftId) public onlyWhenCanRefund {
        ...
        require(
            tokenPaid[ownerOf(nftId)] != address(0),
            "You didn't pay for this token, so you're not getting a refund."
        );

        _burn(nftId);
        address _userPaymentToken = tokenPaid[ownerOf(nftId)];
    ...
    /// @notice List all accepted tokens in the whitelist.
    function getTokensAccepted() external view returns (TokenAndFee[] memory) {
